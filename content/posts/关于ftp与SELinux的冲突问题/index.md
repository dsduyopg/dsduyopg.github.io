...
- [为什么本地能写、FTP 不能写？](#FTP__28)

- [嫌疑一：`write_enable` 没开（最常见！）](#write_enable__41)

- [嫌疑二：SELinux 拦住了 ftpd](#SELinux__ftpd_59)

- [兜底：看日志，让服务器自己告诉你原因](#_78)

- [建议的排查顺序](#_93)

- [2.我看出了什么](#2_102)

- [第 2 步：临时关掉 SELinux 做隔离测试](#_2__SELinux__124)

- [第 3 步：回客户端 lftp 重试](#_3__lftp__131)
...
**1.遇到的问题其实就是，在我用lisi把客户端的根目录下的a.log，发送给服务器的logs目录，无法操作**
 ![在这里插入图片描述](images/31d543a0e0d74a2facd0b57caacd8359.png)
 **2.后来，我在logs目录下创建文件夹也是无法操作的，也是显示权限不足**
 ![在这里插入图片描述](images/55c072190ed842009aa2962c3868c3fc.png)
 但是呢，我在服务器的这个logs目录下创建文件夹是可以创建的
 ![在这里插入图片描述](images/96f60e65512b4a38be2d95aa4d888fd8.png)


`3.就很奇怪，再去服务器是可以搞定的，是可以创建文件夹的，但是在客户端是没有办法进行创建的，我当时以为是logs的权限设置的不对，用的是root创建，权限大的缘故，但是其实并不是如此的,后面我在服务器用了lisi照样是可以创建的`
 ![在这里插入图片描述](images/5596133494e8476a96424bbf30353246.png)
 4.`后来，我仔细检查了这些文件夹的权限，并再次赋权,这次显然也是不行的`
 ![在这里插入图片描述](images/ed81cf0aff1743229e25473bf7b232d7.png)
...
| FTP mkdir | vsftpd 守护进程代为执行 | 受 vsftpd.conf 开关 + SELinux 对 ftpd 的策略双重约束 ❌ |
...
`显然，我是按照setenforce 0 之后，重启服务器，然后就可以了`
 ![在这里插入图片描述](images/f7018adaac4d428abc446956a54d2284.png)

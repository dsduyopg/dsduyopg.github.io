...
- [1.node2配置文件](#1node2_30)

- [2.重启日志文件](#2_34)

- [3.node3文件](#3node3_41)

- [4.node3端口和服务放行](#4node3_45)
...
- [核心原因剖析：用了 TCP，日志为什么还会"凭空消失"？](#_TCP_69)


- [1. 环境层：Cloudflare 客户端重置宿主机网络（根本诱因）](#1_Cloudflare__73)

- [2. 链路层：虚拟机网卡被连带闪断（触发条件）](#2__77)

- [3. 传输/应用层：TCP 握手失败 + rsyslog 默认不缓存（直接原因）](#3_TCP___rsyslog__88)
...
![在这里插入图片描述](images/f2c4680116e04c009aceb90192dcf5d1.png)
...
![在这里插入图片描述](images/cd2a18ecf0fa4b0fb43fdd7ca4db01a2.png)
...
`和node1的文件是同一个，在模块中把tcp协议放行`
 ![在这里插入图片描述](images/328eb6fe34f2489b880e5754d93e6dd6.png)
...
![在这里插入图片描述](images/305a880673d84819930f0a5c64f62a6a.png)
 ![在这里插入图片描述](images/1eeacc31b74c42bdb5bf7b26e2bf8e35.png)
...
`我在测试的时候，发现一个问题，没有通过`
 ![在这里插入图片描述](images/2b40f1fe3dc24151a65e6f137ef2cb29.jpeg)
...
`第二天早上，我仔细有复现了一边，就很离谱，这一次，竟然成功了，配置和昨天一样的，只不过，报了这行文字，我当时想我的虚拟机并没有打开warp这个客户端，为何虚拟机会出现这行文字呢`
 ![在这里插入图片描述](images/d7b11baa23474f31a8e0332cb99ceae2.png)
 **我问了一下，ai她给我说是这样的，因为，我在打开warp客户端的时候，这个软件会修改计算机的路由表，DNS把DNS设置为它自己的：127.0.2.2 和 127.0.2.3（这是 Cloudflare 在本地建的 DNS 代理）,因为昨天我可能设置完setfoce=0之后呢，没有立即生效，需要重启才会，当时我可能来回开关这个软件，导致由表处于"脏状态"，今天冷启动后，WARP 的虚拟接口和路由规则重新初始化，恰好没有干扰到虚拟机的桥接网络。**
 ![在这里插入图片描述](images/c6dea01afadc4db68ec2d754b1e7f788.png)
...
VMware 虚拟机的虚拟网卡依托宿主机网络存在。宿主机网络一重置，虚拟机网卡跟着断。日志里的铁证——node2 和 node3 **同一秒**同时出现：
...
- **rsyslog 默认不缓存断连期间的消息。** rsyslog 转发动作默认使用 Direct 队列，没有配置内存/磁盘缓存队列。连接不可用时，这个窗口期内产生的日志**默认直接丢弃**，网络恢复后也不会补发。
...
![在这里插入图片描述](images/ee62ca4927d347a6a97ba69470a55ad5.png)
 ![在这里插入图片描述](images/5803b8215ea64fb888550df31df074fe.png)
...
转发地址**写 IP 不写主机名**，避免 DNS 故障牵连日志发送；
...
很多初学者（包括曾经的我）在遇到服务不通时，总是习惯性地怀疑自己：“是不是我 rsyslog 配置写错了？”“是不是 SELinux 没关？”“是不是防火墙没放行？”
 但这次事故告诉我：在真实的运维世界里，80% 的“玄学”故障，根源都在你看不见的基础网络环境里。
 一个宿主机代理软件的抽风，足以让你的虚拟机网络经历“闪断-重连”，从而在应用层制造出“配置全对但就是不工作”的假象。能写出正确的配置文件只是基本功，能敏锐地察觉环境变化、熟练运用 tcpdump 抓包定界、懂得在复杂干扰中控制变量，才是一个成熟运维工程师的真正壁垒。
 踩坑不可怕，把坑填平并总结出方法论，就是最大的成长。希望这篇复盘，能帮你在未来的 Linux 学习之路上少走弯路。如果觉得有用，欢迎点赞收藏，我们评论区交流！💪``
